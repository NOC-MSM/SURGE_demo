...
def main(run_dir: str) -> None:
    ...
    # Open existing atm. forcing file and select first 120 hours (5-days):
    ds_atm = xr.open_mfdataset(forcing_fpath)
    ds_atm = ds_atm.isel(t=slice(0, 120))

    # -- Create synthetic storm track -- #
    # Synthetic storm moving zonally across model domain:
    dx_dt = 33/120
    lon_start = ds_domain.nav_lon.isel(y=50, x=0).values
    ...
    rmw = np.repeat(50, len(lon)) # Radius of maximum wind in km
    time = ds_atm['t'].values 
    ...
    storm = h80(df, ds_domain.nav_lon.values, ds_domain.nav_lat.values, B_model='vickery00')
    ...
    # Collect first 5-days of SETTE AMM12 atm. forcing filenames:
    fnames = sorted(glob.glob(f"{run_dir}/fluxes/amm12_utau_y2012m01d*.nc"))[:5]

    # Define output filenames:
    out_fnames = [f.replace('amm12_', 'amm12_surge_') for f in fnames]

    # Iterate over 5-days of existing files and create idealised atm. forcing files:
    for n, f in enumerate(fnames):
        # Construct dataset from synthetic storm data:
        ds_n = xr.Dataset(data_vars={"utau": (['t', 'y', 'x'], storm.data.isel(time=slice(n*24, n*24 + 24)).stress_u.data)},
    ...
    # Collect first 5-days of SETTE AMM12 atm. forcing filenames:
    fnames = sorted(glob.glob(f"{run_dir}/fluxes/amm12_vtau_y2012m01d*.nc"))[:5]
    ...
    for n, f in enumerate(fnames):
        # Construct dataset from synthetic storm data:
        ds_n = xr.Dataset(data_vars={"vtau": (['t', 'y', 'x'], storm.data.isel(time=slice(n*24, n*24 + 24)).stress_v.data)},
                        coords={"nav_lat": (['y', 'x'], ds_domain.nav_lat.squeeze().data),
                                "nav_lon": (['y', 'x'], ds_domain.nav_lon.squeeze().data),
                                "t": ds_atm["t"].isel(t=slice(n*24, n*24 + 24)).data
                                })
    ...
    for n, f in enumerate(fnames):
        ...
        somslpre = storm.data.isel(time=slice(n*24, n*24 + 24)).pressure * 100  # Convert hPa to Pa

        # Construct dataset from synthetic storm data:
        ds_n = xr.Dataset(data_vars={"somslpre": (['t', 'y', 'x'], somslpre.data)},
                        coords={"nav_lat": (['y', 'x'], ds_domain.nav_lat.squeeze().data),
                                "nav_lon": (['y', 'x'], ds_domain.nav_lon.squeeze().data),
                                "t": ds_atm["t"].isel(t=slice(n*24, n*24 + 24)).data
                                })
